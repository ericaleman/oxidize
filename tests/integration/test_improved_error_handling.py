...
def test_empty_target_element_error():
    ...
    with pytest.raises(ValueError) as exc_info:
        oxidize.parse_xml_string_to_json_string(xml_content, "")
...
def test_zero_batch_size_error():
    ...
    with pytest.raises(ValueError) as exc_info:
        oxidize.parse_xml_string_to_json_string(xml_content, "item", batch_size=0)
...
def test_huge_batch_size_error():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        oxidize.parse_xml_string_to_json_string(xml_content, "item", batch_size=2_000_000)
...
def test_file_not_found_error():
    ...
    with pytest.raises(OSError) as exc_info:
        oxidize.parse_xml_file_to_json_string("/nonexistent/path/file.xml", "item")
...
def test_invalid_output_path_error():
    ...
    with pytest.raises(OSError) as exc_info:
        oxidize.parse_xml_string_to_json_file(xml_content, "item", "/invalid/path/output.json")
...
def test_malformed_xml_error():
    ...
    try:
        result = oxidize.parse_xml_string_to_json_string(malformed_xml, "item")
...
def test_xml_parse_error_with_position():
    ...
    try:
        result = oxidize.parse_xml_string_to_json_string(invalid_xml, "item")
...
def test_error_context_preservation():
    ...
    with pytest.raises(ValueError) as exc_info:
        oxidize.parse_xml_file_to_json_string("valid_file.xml", "", batch_size=500)
...
def test_all_functions_have_consistent_error_handling(temp_dir):
    ...
    functions_to_test = [
        lambda: oxidize.parse_xml_string_to_json_string(xml_content, "", batch_size=1),
        lambda: oxidize.parse_xml_string_to_json_file(xml_content, "", str(temp_dir / "out.json"), batch_size=1),
        lambda: oxidize.parse_xml_file_to_json_string(str(xml_path), "", batch_size=1),
        lambda: oxidize.parse_xml_file_to_json_file(str(xml_path), "", str(temp_dir / "out2.json"), batch_size=1),
    ]
...
def test_memory_error_handling():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        oxidize.parse_xml_string_to_json_string(xml_content, "item", batch_size=5_000_000)
...
def test_io_error_handling(temp_dir):
    ...
    try:
        with pytest.raises(OSError) as exc_info:
            oxidize.parse_xml_string_to_json_file(
...
def test_utf8_conversion_error_handling():
    ...
    try:
        result = oxidize.parse_xml_string_to_json_string(xml_content, "item")
        assert isinstance(result, str)
...
def test_error_message_formatting():
    ...
    test_cases = [
        ...
        (
            lambda: oxidize.parse_xml_string_to_json_string('<root></root>', ''),
            ValueError,
            ["Invalid input", "target_element cannot be empty", "context"]
        ),
        (
            lambda: oxidize.parse_xml_string_to_json_string('<root></root>', 'item', batch_size=0),
            ValueError,
            ["Invalid input", "batch_size must be greater than 0", "Received batch_size: 0"]
        ),
        (
            lambda: oxidize.parse_xml_file_to_json_string('/does/not/exist.xml', 'item'),
            OSError,
